...
public class Main {
	...
	// ------------------------- HELP FUNCTIONS ------------------//
	private static void print2DArray(Double[][] calculateEuclideanDistance) {
		// numbers of columns
		for (int x = 0; x <= calculateEuclideanDistance.length; x++) {
			if (x > 0)
				System.out.print("\t     [" + x + "]");
		}
		System.out.println();
		for (int i = 0; i < calculateEuclideanDistance.length; i++) {
			...
			for (int j = 0; j < calculateEuclideanDistance.length; j++) {
				System.out.print(df.format(calculateEuclideanDistance[i][j]) + "\t");
			}
			System.out.println();
		}
	}
